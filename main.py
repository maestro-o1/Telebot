...
Muallif: @maestro_o
Versiya: 4.2 - Tuzatilgan (vaqt mintaqasi, kategoriyalar, rasm ID, ruxsat so'rovi, filial atamasi)
"""
...
class PriceHistory(Base):
    ...
    changed_at = Column(DateTime, default=lambda: datetime.now(TASHKENT_TZ))
...
def init_db():
    ...
    try:
        ...
        # Kategoriyalarni yaratish (endi default yo'q)
        # Faqat admin qo'shgan kategoriyalar bo'ladi
...
def backup_database():
    ...
    backup_name = f"data/backup_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.db"
...
def get_main_keyboard(user_is_admin=False, user_branch_type=None, user_branch_name=None):
    ...
    if user_is_admin:
        buttons.extend([
            [InlineKeyboardButton("📊 Filiallar", callback_data="layers")],
        ...
        buttons.extend([
            ...
            [InlineKeyboardButton(f"➕ {user_branch_name} uchun tovar qo'shish", callback_data="add_product_parasite")],
        ])
    else:
        buttons.extend([
            [InlineKeyboardButton(f"➕ Tovar qo'shish", callback_data="add_product_user")],
        ])
...
def get_back_main_buttons(callback_data="main_menu"):
...
def get_cancel_skip_buttons(callback_data="main_menu"):
    """Bekor qilish va O'tkazib yuborish tugmalari"""
    return InlineKeyboardMarkup([
        [
...
def get_layers_keyboard():
    """Filiallar menyusi"""
    buttons = [
        [InlineKeyboardButton("👑 Asosiy filial", callback_data="layer_main")],
        [InlineKeyboardButton("🐝 Chiroqchi filiali", callback_data="layer_chiroqchi")],
        [InlineKeyboardButton("🏠 Bosh menyu", callback_data="main_menu")]
    ]
    return InlineKeyboardMarkup(buttons)
...
def get_user_actions_keyboard(user_id):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🔄 Filialni o'zgartirish", callback_data=f"change_branch_{user_id}")],
...
def get_branch_keyboard(user_id):
    """Filial tanlash"""
    buttons = [
        [InlineKeyboardButton("👑 Asosiy filial (Admin)", callback_data=f"set_branch_main_{user_id}")],
        [InlineKeyboardButton("🐝 Chiroqchi filiali", callback_data=f"set_branch_chiroqchi_{user_id}")],
        [InlineKeyboardButton("🏛 Shahrisabz filiali", callback_data=f"set_branch_shahrisabz_{user_id}")],
...
async def export_to_excel():
    ...
    for p in products:
        ...
        data.append({
            ...
            'Filial': 'Asosiy' if p.owner_type == 'main' else owner_name,
    ...
    filename = f"data/export_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.xlsx"
        ...
        for _, row in df.iterrows():
            ...
            if category_name and category_name != 'nan' and category_name.strip():
                if category_name not in categories_cache:
                    category = session.query(Category).filter_by(name=category_name.strip()).first()
                    if not category:
            ...
            owner_type = 'main'
            owner_id = None
            filial = str(row.get('Filial', 'Asosiy'))
            if filial != 'Asosiy':
                owner = session.query(User).filter_by(branch_name=filial).first()
                if owner:
                    owner_type = 'parasite'
                    owner_id = owner.id
...
async def get_product_info_text(product, owner_name="Asosiy"):
    ...
    text += f"📊 <b>Filial:</b> {'Asosiy' if product.owner_type == 'main' else 'Chiroqchi'}\n\n"
    ...
    try:
        ...
        if db_user.is_admin:
            ...
            text = (f"👋 Xush kelibsiz Admin!\n\n"
                    f"📊 <b>FILIALLAR STATISTIKASI</b>\n"
                    f"👑 Asosiy filial: {main_products} ta tovar\n")
            ...
            if chiroqchi_users:
                for cu in chiroqchi_users:
                    ...
                    text += f"🐝 {cu.branch_name} filiali: {user_products} ta tovar\n"
            else:
                text += f"🐝 Chiroqchi filiali: {chiroqchi_products} ta tovar\n"
            
            text += f"\n📌 <b>Filiallar haqida:</b>\n"
            text += f"• Asosiy filial - siz va Shahrisabz ko'rasiz\n"
            text += f"• Chiroqchi filiali - faqat Chiroqchi o'zi ko'radi\n\n"
        ...
        logger.error(f"Show main menu xatolik: {e}")
        if update.callback_query:
            await update.callback_query.edit_message_text("❌ Xatolik yuz berdi. Iltimos, qaytadan urinib ko'ring.")
        else:
...
<b>📊 Filiallar (Admin):</b>
• 👑 Asosiy filial - Admin va Shahrisabz
• 🐝 Chiroqchi filiali - faqat Chiroqchi

<b>➕ Tovar qo'shish:</b>
• Nomi, rasmi, narxlari
• USD yoki UZS tanlash
• <b>O'tkazib yuborish</b> - istalgan qadamni tashlab ketish
    ...
    try:
        ...
        if data == "skip":
            ...
            if state == ADD_PRODUCT_PHOTO:
                context.user_data['new_product']['media_message_id'] = None
                context.user_data['new_product']['media_file_id'] = None
                context.user_data['state'] = ADD_PRODUCT_PURCHASE_USD
                await query.edit_message_text(
                    "💰 Kelgan narxini kiriting ($) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_PURCHASE_USD:
                context.user_data['new_product']['purchase_usd'] = 0
                context.user_data['state'] = ADD_PRODUCT_PURCHASE_UZS
                await query.edit_message_text(
                    "💰 Kelgan narxini kiriting (so'm) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_PURCHASE_UZS:
                context.user_data['new_product']['purchase_uzs'] = 0
                context.user_data['state'] = ADD_PRODUCT_SELLING_USD
                await query.edit_message_text(
                    "💵 Sotilish narxini kiriting ($) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_SELLING_USD:
                context.user_data['new_product']['selling_usd'] = 0
                context.user_data['state'] = ADD_PRODUCT_SELLING_UZS
                await query.edit_message_text(
                    "💵 Sotilish narxini kiriting (so'm) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_SELLING_UZS:
                context.user_data['new_product']['selling_uzs'] = 0
                context.user_data['state'] = ADD_PRODUCT_QUANTITY
                await query.edit_message_text(
                    "📦 Soni (dona) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_QUANTITY:
                context.user_data['new_product']['quantity'] = 0
                context.user_data['state'] = ADD_PRODUCT_KEYWORDS
                await query.edit_message_text(
                    "🔑 Kalit so'zlar (vergul bilan) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            
            elif state == ADD_PRODUCT_KEYWORDS:
                context.user_data['new_product']['keywords'] = ""
                # Kategoriya tanlashga o'tish
                await show_category_selection(update, context, session)
        ...
        if data == "search":
            ...
            await query.edit_message_text(
                ...
                reply_markup=get_back_main_buttons()
            )
            return
        ...
        if data.startswith("approve_req_") or data.startswith("reject_req_"):
            ...
            if data.startswith("approve_req_"):
                ...
                # Qatlam tanlash uchun keyboard ko'rsatamiz
                await query.edit_message_text(
                    f"👤 {target_user.first_name} uchun filialni tanlang:",
                    reply_markup=get_branch_keyboard(target_user.id)
                )
                request.status = 'approved'
                session.commit()
                logger.info(f"So'rov qabul qilindi, filial tanlash kutilmoqda: {target_user.id}")
            else:
                request.status = 'rejected'
                session.commit()
                logger.info(f"So'rov rad etildi: {request.user_id}")
                ...
                await query.edit_message_text(
                    f"❌ Foydalanuvchi {target_user.first_name if target_user else 'Noma\'lum'} rad etildi!",
                    reply_markup=get_back_main_buttons("users")
                )
            return
        ...
        if data == "my_products":
            ...
            if not products:
                ...
                await query.edit_message_text(text, reply_markup=get_back_main_buttons(), parse_mode='HTML')
                return
        ...
        # Kategoriya tanlash (tovar qo'shishda)
        if data.startswith("select_cat_"):
            category_id = int(data.split("_")[2])
            await save_product_with_category(update, context, session, category_id)
            return
        
        # Yangi kategoriya qo'shish (tovar qo'shishda)
...
async def show_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, session):
    """Kategoriya tanlashni ko'rsatish"""
    query = update.callback_query
...
async def save_product_with_category(update: Update, context: ContextTypes.DEFAULT_TYPE, session, category_id):
    """Tovarni kategoriya bilan saqlash"""
    query = update.callback_query
    ...
    try:
        ...
        product = Product(
            ...
            media_file_id=context.user_data['new_product'].get('media_file_id'),
            is_active=True
        )
        session.add(product)
        session.flush()  # ID olish uchun
        
        # Rasmni guruhga ID bilan yuborish
        if product.media_file_id and group:
            try:
                # Rasmni guruhga tovar ID si bilan yuborish
                caption = f"📦 Tovar ID: {product.id}\nNomi: {product.name}"
                sent_message = await context.bot.send_photo(
                    chat_id=group.group_id,
                    photo=product.media_file_id,
                    caption=caption
                )
                # Rasm ID sini yangilash
                product.media_group_message_id = sent_message.message_id
                session.commit()
                logger.info(f"Rasm saqlandi, Tovar ID: {product.id}, Message ID: {sent_message.message_id}")
            except Exception as e:
                logger.error(f"Rasmni guruhga yuborishda xatolik: {e}")
        ...
        await query.edit_message_text(
            ...
            f"🆔 Tovar ID: {product.id}\n"
            f"👤 Egasi: {owner_text}\n"
            f"📊 Filial: {'Asosiy' if product.owner_type == 'main' else 'Chiroqchi'}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Orqaga", callback_data=f"cat_{category_id}")],
                [InlineKeyboardButton("🏠 Bosh menyu", callback_data="main_menu")]
            ])
        )
        logger.info(f"Yangi tovar qo'shildi: {product.name} - ID: {product.id}")
        ...
        logger.error(f"Tovarni saqlashda xatolik: {e}")
        await query.edit_message_text(f"❌ Xatolik: {str(e)}")
...
async def category_detail_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user, category_id):
    ...
    if products:
        ...
        for p in products:
            emoji = "👑" if p.owner_type == 'main' else "🐝"
            text += f"• {emoji} {p.name} (ID: {p.id}) - {p.quantity} dona\n"
...
async def add_category_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    ...
    await query.edit_message_text(
        "📝 Yangi kategoriya nomini kiriting:",
        reply_markup=get_cancel_skip_buttons(f"cat_{parent_id}")
    )
...
async def add_product_start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    ...
    # Ruxsatni tekshirish
    if not db_user.is_authorized and not db_user.is_admin:
        await query.edit_message_text("❌ Tovar qo'shish uchun ruxsat kerak!")
        return
    
    if data == "add_product":
        # Admin uchun
        context.user_data['new_product']['owner_type'] = 'main'
        context.user_data['new_product']['owner_id'] = None
        context.user_data['new_product']['branch'] = 'main'
        back_callback = "main_menu"
    
    elif data == "add_product_user":
        # Oddiy foydalanuvchi uchun (faqat asosiy filialga)
    ...
    elif data == "add_product_parasite":
        # Parazit uchun
        if db_user.branch_type != 'parasite':
            await query.edit_message_text("❌ Bu bo'lim faqat Chiroqchi uchun!")
            return
        context.user_data['new_product']['owner_type'] = 'parasite'
        context.user_data['new_product']['owner_id'] = db_user.id
        context.user_data['new_product']['branch'] = db_user.branch_name
        back_callback = "main_menu"
    
    elif len(parts) >= 4 and parts[2] == 'cat':
        # Kategoriyadan qo'shish
        ...
        context.user_data['new_product']['branch'] = 'main' if db_user.is_admin else db_user.branch_name
        back_callback = "main_menu"
    ...
    await query.edit_message_text(
        ...
        reply_markup=get_cancel_skip_buttons(back_callback)
    )
    ...
    if len(parts) == 2:
        ...
        if field == 'purchase':
            ...
            await query.edit_message_text(
                ...
                reply_markup=get_cancel_skip_buttons(f"view_{product_id}")
            )
...
async def set_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    ...
    await query.edit_message_text(
        ...
        reply_markup=get_cancel_skip_buttons(f"edit_{product_id}")
    )
...
async def layer_main_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    """Asosiy filial tovarlarini ko'rsatish"""
    query = update.callback_query
    
    try:
        # Asosiy filial tovarlari
        products = session.query(Product).filter_by(owner_type='main', is_active=True).all()
        
        text = "👑 <b>ASOSIY FILIAL</b>\n\n"
        text += f"Jami tovarlar: {len(products)} ta\n\n"
        
        if not products:
            text += "Asosiy filialda tovarlar yo'q."
            await query.edit_message_text(text, reply_markup=get_back_main_buttons("layers"), parse_mode='HTML')
            return
        ...
        for cat_name, cat_products in categories.items():
            text += f"\n📂 <b>{cat_name}</b> ({len(cat_products)} ta):\n"
            for p in cat_products[:5]:
                text += f"   • {p.name} (ID: {p.id}) - {p.quantity} dona\n"
            if len(cat_products) > 5:
                text += f"     ... va yana {len(cat_products) - 5} ta\n"
        ...
        buttons = []
        for p in products[:10]:
            buttons.append([InlineKeyboardButton(f"👑 {p.name} (ID: {p.id})", callback_data=f"view_{p.id}")])
...
async def layer_chiroqchi_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    """Chiroqchi filiali tovarlarini ko'rsatish"""
    query = update.callback_query
    ...
    try:
        ...
        for p in all_parasite[:10]:
            owner = session.query(User).filter_by(id=p.owner_id).first()
            owner_name = owner.branch_name if owner else "Noma'lum"
            buttons.append([InlineKeyboardButton(f"🐝 [{owner_name}] {p.name} (ID: {p.id})", callback_data=f"view_{p.id}")])
...
async def approve_user_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    ...
    if target_user:
        # Avval filial tanlash uchun keyboard ko'rsatamiz
        await query.edit_message_text(
            f"👤 {target_user.first_name} uchun filialni tanlang:",
            reply_markup=get_branch_keyboard(target_user.id)
        )
...
async def change_branch_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    """Foydalanuvchi filialini o'zgartirish"""
    query = update.callback_query
    data = query.data
...
async def set_branch_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    """Foydalanuvchi filialini o'rnatish"""
    query = update.callback_query
    data = query.data
    parts = data.split("_")
    ...
    if target_user:
        ...
        session.commit()
        logger.info(f"Filial o'zgartirildi: {target_user.id} -> {branch}")
        
        await query.edit_message_text(
            f"✅ {target_user.first_name} uchun filial o'zgartirildi: {branch}",
            reply_markup=get_back_main_buttons("users")
        )
        ...
        try:
            await context.bot.send_message(
                target_user.telegram_id,
                f"✅ Sizning filialingiz o'zgartirildi: {branch}\n"
                f"/start ni bosing."
            )
        except Exception as e:
            logger.error(f"Xabar yuborishda xatolik: {e}")
...
async def stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, session, db_user):
    ...
    text = (f"📊 <b>STATISTIKA</b>\n\n"
            ...
            f"👑 Asosiy filial: {main_products} ta\n"
            f"🐝 Chiroqchi filiali: {parasite_products} ta\n\n"
    ...
    text += f"\n🕐 Yangilangan: {datetime.now(TASHKENT_TZ).strftime('%H:%M:%S %d.%m.%Y')}"
    ...
    try:
        ...
        with open(filename, 'rb') as f:
            await context.bot.send_document(
                chat_id=update.effective_user.id,
                document=f,
                filename=f"tovarlar_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.xlsx",
                caption="✅ Tovarlar ro'yxati (barcha filiallar bilan)"
            )
    ...
    try:
        ...
        with open(backup_file, 'rb') as f:
            await context.bot.send_document(
                chat_id=update.effective_user.id,
                document=f,
                filename=f"backup_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.db",
    ...
    try:
        ...
        if state == ADD_PRODUCT_NAME:
            if not text or text.strip() == "":
                await update.message.reply_text("❌ Tovar nomi bo'sh bo'lishi mumkin emas!")
                return
            context.user_data['new_product']['name'] = text.strip()
            context.user_data['state'] = ADD_PRODUCT_PHOTO
            await update.message.reply_text(
                "🖼 Tovar rasmini yuboring (yoki o'tkazib yuboring):",
                reply_markup=get_cancel_skip_buttons()
            )
            return
        ...
        if state == ADD_PRODUCT_PURCHASE_USD:
            try:
                value = float(text.replace(',', '.')) if text.strip() else 0
                ...
                await update.message.reply_text("❌ Noto'g'ri format! Qayta kiriting yoki o'tkazib yuboring:")
            return
        ...
        if state == ADD_PRODUCT_QUANTITY:
            try:
                value = int(text) if text.strip() else 0
                context.user_data['new_product']['quantity'] = value
                context.user_data['state'] = ADD_PRODUCT_KEYWORDS
                await update.message.reply_text(
                    "🔑 Kalit so'zlar (vergul bilan ajrating) yoki o'tkazib yuboring:",
                    reply_markup=get_cancel_skip_buttons()
                )
            except ValueError:
                await update.message.reply_text("❌ Noto'g'ri format! Qayta kiriting yoki o'tkazib yuboring:")
        ...
        if state == ADD_PRODUCT_KEYWORDS:
            context.user_data['new_product']['keywords'] = text.strip() if text.strip() else ""
        ...
        if state == ADD_PRODUCT_CATEGORY:
            ...
            if existing:
                ...
                await update.message.reply_text(f"ℹ️ Bu kategoriya allaqachon mavjud.")
            else:
            ...
            if 'new_product' in context.user_data:
                # Tovarni saqlash
                await save_product_with_category_from_message(update, context, session, category_id)
            else:
        ...
        if state == EDIT_WAITING:
            ...
            if field == 'name':
                ...
                try:
                    product.quantity = int(text) if text.strip() else 0
                except ValueError:
                    await update.message.reply_text("❌ Noto'g'ri format!")
                    return
            elif field == 'keywords':
                product.keywords = text.strip() if text.strip() else ""
        ...
        if state == GRANT_ACCESS_WAITING:
            try:
                ...
                if target_user:
                    # Filial tanlash uchun keyboard ko'rsatamiz
                    await update.message.reply_text(
                        f"👤 {target_user.first_name} uchun filialni tanlang:",
                        reply_markup=get_branch_keyboard(target_user.id)
                    )
                    logger.info(f"Ruxsat berish jarayoni: {target_id}")
                else:
                    await update.message.reply_text(
...
async def save_product_with_category_from_message(update: Update, context: ContextTypes.DEFAULT_TYPE, session, category_id):
    """Xabardan tovarni kategoriya bilan saqlash"""
    ...
    try:
        ...
        if state == ADD_PRODUCT_PHOTO:
            try:
                ...
                # Rasm file_id sini saqlaymiz (keyinroq guruhga yuborish uchun)
                ...
                logger.info(f"Rasm saqlandi (kutish rejimi): {user.id}")
                
            except Exception as e:
                logger.error(f"Rasm qabul qilishda xatolik: {e}")
                await update.message.reply_text(
            ...
            if field == 'photo':
                ...
                if product:
                    try:
                        ...
                        # Rasm file_id sini saqlash
                        product.media_file_id = photo.file_id
...
async def document_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if state == IMPORT_WAITING and (file_name.endswith('.xlsx') or file_name.endswith('.xls')):
            ...
            filename = f"data/import_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.xlsx"
        ...
        # Backup fayl tiklash
            ...
            file = await document.get_file()
            filename = f"data/restore_{datetime.now(TASHKENT_TZ).strftime('%Y%m%d_%H%M%S')}.db"
            await file.download_to_drive(filename)
    ...
    try:
        if update and update.effective_chat:
            await context.bot.send_message(
                ...
                "❌ Xatolik yuz berdi. Admin bilan bog'langan."
            )
    except Exception as e:
...
def main():
    ...
    print("="*50)
    print(f"🕐 Vaqt: {datetime.now(TASHKENT_TZ).strftime('%H:%M:%S')}")
    print(f"📅 Sana: {datetime.now(TASHKENT_TZ).strftime('%d.%m.%Y')}")
    print("="*50 + "\n")
